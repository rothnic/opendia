...
const enableTunnel =
	args.includes("--tunnel") || args.includes("--auto-tunnel");
...
async function findAvailablePort(startPort) {
	...
	while (await checkPortInUse(port)) {
		...
		if (port > startPort + 100) {
			// Safety limit
			throw new Error(
				`Could not find available port after checking ${port - startPort} ports`,
			);
		}
	}
	return port;
...
async function handlePortConflict(port, portName) {
	...
	if (isOpenDia) {
		...
		if (portName === "WebSocket") {
			console.error(
				`💡 Update Chrome/Firefox extension to: ws://localhost:${altPort}`,
			);
		}
		return altPort;
	} else {
		...
		const altPort = await findAvailablePort(port + 1);
		console.error(
			`🔄 ${portName} port ${port} busy (non-OpenDia), using port ${altPort}`,
		);
		if (portName === "WebSocket") {
			console.error(
				`💡 Update Chrome/Firefox extension to: ws://localhost:${altPort}`,
							...
							arguments: [
								{
									name: "form_type",
									description:
										"Type of form: contact, registration, survey, application",
									required: false,
								},
							],
						},
				...
				try {
					...
					switch (promptName) {
						...
						case "post_selected_quote":
							workflowResult =
								await executePostSelectedQuoteWorkflow(promptArgs);
							break;
...
async function startServer() {
	...
	if (HTTP_PORT === WS_PORT) {
		...
		console.error(
			`🔄 HTTP port adjusted to ${HTTP_PORT} to avoid WebSocket conflict`,
		);
	}
	...
	const httpServer = app.listen(HTTP_PORT, () => {
		...
		console.error(
			`🔌 Browser Extension connected on ws://localhost:${WS_PORT}`,
		);
		console.error(
			"🎯 Features: Anti-detection bypass + intelligent automation",
		);
	});
	...
	if (enableTunnel) {
		try {
			...
			if (tunnelUrl) {
				...
				console.error(
					"💡 Claude Web: Add as external MCP server (if supported)",
				);
				console.error("");
	...
	if (sseOnly) {
		...
		console.error(
			`💡 Configure Claude Desktop with: http://localhost:${HTTP_PORT}/sse`,
		);
	} else {
...
async function executePostSelectedQuoteWorkflow(args) {
	...
	try {
		...
		if (!selectedText.has_selection) {
			throw new Error(
				"No text is currently selected. Please select some text first.",
			);
		}
...
async function executeResearchWorkflow(args) {
	...
	try {
		...
		result += `\n📚 **Previous Research:**\n`;
		if (
			historyResults.history_items &&
			historyResults.history_items.length > 0
		) {
...
async function executeSessionAnalysisWorkflow(args) {
	...
	try {
		...
		if (
			categorizedTabs.productivity.length > 0 &&
			categorizedTabs.social.length > 0
		) {
	...
	try {
		...
		if (formElements.inputs.length > 0) {
			...
			formElements.inputs.forEach((input, index) => {
				const elementState = formAnalysis.elements.find(
					(el) => el.id === input.id,
				);
