...
const browserInfo = {
	isFirefox:
		...
		browser.runtime.getManifest().applications?.gecko,
	isChrome:
	...
	async createConnection() {
		...
		this.mcpSocket.onclose = (event) => {
			...
			if (event.code !== 1000 && event.code !== 1001) {
				console.log(
					"🔄 Abnormal WebSocket closure, attempting reconnection...",
				);
...
async function ensureContentScriptReady(tabId, retries = 3) {
			...
			if (attempt === retries) {
				...
				try {
					...
					if (!isInjectableUrl(tab.url)) {
						throw new Error(
							`Cannot inject content script into ${tab.url} - restricted URL`,
						);
					}
					...
					if (browser.scripting) {
						...
						try {
							...
							if (result && result[0]) {
								console.log(
									`🔄 Content script already present in tab ${tabId}`,
								);
								return true;
							}
						} catch (e) {
					...
					const testResponse = await new Promise((resolve, reject) => {
						const timeout = setTimeout(
							() => reject(new Error("Timeout after injection")),
							3000,
						);
						browser.tabs.sendMessage(tabId, { action: "ping" }, (response) => {
							clearTimeout(timeout);
					...
					if (testResponse && testResponse.success) {
						console.log(
							`✅ Content script successfully injected into tab ${tabId}`,
						);
						return true;
					}
	...
	throw new Error(
		`Content script not available in tab ${tabId} after ${retries} attempts`,
	);
}
...
function getAvailableTools() {
    ...
    */
			...
			inputSchema: {
				...
				properties: {
					...
					focus_areas: {
						...
						items: { type: "string" },
						description:
							"Areas to analyze in detail: buttons, forms, navigation, search_elements",
					},
					max_results: {
						type: "number",
						default: 5,
					...
					element_ids: {
						...
						items: { type: "string" },
						description:
							"Expand specific quick match IDs from discover phase (e.g. ['q1', 'q2'])",
					},
					tab_id: {
						type: "number",
						description:
			...
			inputSchema: {
				...
				properties: {
					...
					max_items: {
						type: "number",
						description:
							"Maximum number of items to extract (for lists/collections)",
						default: 20,
					},
					summarize: {
						type: "boolean",
			...
			inputSchema: {
				...
				properties: {
					...
					force_focus: {
						type: "boolean",
						description:
							"Use enhanced focus sequence with click simulation for modern apps",
						default: true,
					},
					tab_id: {
						type: "number",
			...
			inputSchema: {
				...
				properties: {
					...
					wait_for: {
						type: "string",
						description:
							"CSS selector to wait for after navigation (ensures page is ready)",
					},
					timeout: {
						type: "number",
			...
			inputSchema: {
				...
				properties: {
					...
					text: {
						type: "string",
						description:
							"Text to wait for (when condition_type is 'text_present')",
					},
					timeout: {
						type: "number",
						description: "Maximum wait time in milliseconds",
			...
			inputSchema: {
				...
				properties: {
					...
					active: {
						...
						default: true,
						description:
							"Whether to activate the last created tab (single tab only)",
					},
					wait_for: {
						type: "string",
						description:
							"CSS selector to wait for after tab creation (single tab only)",
					},
					timeout: {
						type: "number",
						default: 10000,
			...
			inputSchema: {
				...
				properties: {
					tab_id: {
						type: "number",
						description:
							"Specific tab ID to close (optional, closes current tab if not provided)",
					},
					tab_ids: {
						type: "array",
						items: { type: "number" },
			...
			inputSchema: {
				...
				properties: {
					...
					include_details: {
						...
						default: true,
						description:
							"Include additional tab details (title, favicon, etc.)",
					},
			...
			inputSchema: {
				...
				properties: {
					...
					domains: {
						...
						items: { type: "string" },
						description:
							"Filter by specific domains (e.g., ['github.com', 'stackoverflow.com'])",
					},
					min_visit_count: {
						type: "number",
						default: 1,
			...
			inputSchema: {
				...
				properties: {
					include_metadata: {
						...
						default: true,
						description:
							"Include metadata about the selection (element info, position, etc.)",
					},
					max_length: {
						type: "number",
						default: 10000,
			...
			inputSchema: {
				...
				properties: {
					...
					element_id: {
						type: "string",
						description:
							"Scroll to specific element (overrides direction/amount)",
					},
					wait_after: {
						type: "number",
						default: 500,
		...
		},
		{
			name: "get_page_links",
			description:
				"Get all hyperlinks on the current page with filtering options",
			inputSchema: {
				type: "object",
				properties: {
					link_type: {
			...
			inputSchema: {
				...
				properties: {
					...
					include_context: {
						...
						default: true,
						description:
							"Include browser context (URL, title, tab info) in the response",
					},
				},
				required: ["script"],
			},
...
async function handleMCPRequest(message) {
	...
	try {
		...
		if (safetyModeEnabled && WRITE_EDIT_TOOLS.includes(method)) {
			const targetInfo = params.tab_id
				? `tab ${params.tab_id}`
				: "the current page";
			throw new Error(
	...
	return new Promise((resolve, reject) => {
		browser.tabs.sendMessage(targetTab.id, { action, data }, (response) => {
			if (browser.runtime.lastError) {
				reject(
					new Error(
						`Tab ${targetTab.id}: ${browser.runtime.lastError.message}`,
					),
				);
				...
				reject(
					new Error(
						`Tab ${targetTab.id}: ${response?.error || "Unknown error"}`,
					),
				);
			}
		});
...
async function createTabsBatch(
	urls,
	active,
	wait_for,
	timeout,
	...
	console.log(
		`🚀 Starting batch tab creation: ${totalTabs} tabs in chunks of ${chunk_size}`,
	);
	...
	for (let chunkStart = 0; chunkStart < urls.length; chunkStart += chunk_size) {
		...
		for (let i = 0; i < chunk.length; i++) {
			...
			try {
				...
				if (totalTabs <= 5) {
					console.log(
						`✅ Created tab ${globalIndex + 1}/${totalTabs}: ${url} (ID: ${tab.id})`,
					);
				}
				...
				if (i < chunk.length - 1) {
					await new Promise((resolve) =>
						setTimeout(resolve, delay_between_tabs),
					);
				}
			} catch (error) {
				console.error(
					`❌ Failed to create tab ${globalIndex + 1}: ${error.message}`,
				);
				errors.push({
					index: globalIndex,
					url: url,
		...
		if (chunkEnd < urls.length) {
			if (totalTabs <= 10) {
				console.log(
					`⏳ Waiting ${delay_between_chunks}ms before next chunk...`,
				);
			}
...
async function listTabs(params) {
	...
	if (check_content_script) {
		const statusPromises = tabs.map(async (tab) => {
			try {
				...
				return [
					tab.id,
					{ ready: false, reason: "error", error: error.message },
				];
			}
		});
	...
	if (check_content_script) {
		...
		const injectableTabs = tabList.filter(
			(tab) => tab.content_script?.injectable,
		).length;
...
async function executeScriptInTab(params) {
	...
	try {
		...
		// Extract the result
		const scriptResult =
			results[0]?.result !== undefined ? results[0].result : results[0];
